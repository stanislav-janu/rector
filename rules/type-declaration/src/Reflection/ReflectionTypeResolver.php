...
final class ReflectionTypeResolver
{
    ...
    public function resolveFuncCallReturnType(FuncCall $funcCall): ?Type
    {
        ...
        if ($funcName instanceof Expr) {
            return null;
        }

        if (! $this->reflectionProvider->hasFunction($funcCall->name, $funcCallScope)) {
            return null;
        }

        $functionReflection = $this->reflectionProvider->getFunction($funcCall->name, $funcCallScope);
